...
//        int bucket_num = murmurHash(reinterpret_cast<const void*>(&key), sizeof(key), top_index->f_seed) % top_index->bucket_num;
        ...
        bool remove(const Key key) {
            int bucket_num = murmurHash(reinterpret_cast<const void *>(&key), sizeof(key), top_index->f_seed) %
                             top_index->bucket_num;
            struct bucketInfo *bucket = top_index->buckets + bucket_num;
            int item_index = bucket->begin_index;
        ...
        long long getBucketNumber(){
            return top_index->bucket_num;
        }

        long long getItemNumber(){
            return top_index->item_num;
        }
